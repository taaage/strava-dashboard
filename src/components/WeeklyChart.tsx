...
function getWeeklyData(activities: StravaActivity[]) {
...
export function WeeklyChart({ activities }: WeeklyChartProps) {
  const data = getWeeklyData(activities);
  ...
  return (
    <div className="bg-surface-card rounded-3xl p-8 border border-surface-border">
      ...
      <p className="text-sm text-text-muted mb-6">
        Last 12 weeks · outdoor vs indoor
      </p>
      <div className="h-56">
      ...
      <div className="flex items-center justify-center gap-6 mt-4 text-xs text-text-muted">
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-sm bg-[hsl(221,83%,53%)] inline-block" />{" "}
          Outdoor
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-sm bg-[hsl(212,95%,68%)] inline-block" />{" "}
          Indoor
        </span>
      </div>
    </div>
