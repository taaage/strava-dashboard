...
export function EfficiencyAreaChart({ activities }: EfficiencyAreaChartProps) {
  ...
  const rides = activities
    .filter(
      (a) =>
        ...
        a.average_watts > 0 && a.average_heartrate > 0 &&
  ...
  return (
    <ChartCard title="Efficiency Over Time" subtitle="Watts per heartbeat — higher = fitter" timeRange={timeRange}>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={rides}>
          ...
          <YAxis axisLine={false} tickLine={false} tick={{ fill: "#71717a", fontSize: 11 }} domain={["auto", "auto"]} dx={-4} />
          <Tooltip
            content={({ active, payload }) => {
              ...
              return (
                <div className="bg-surface-muted rounded-lg px-3 py-2 border border-surface-border shadow-xl">
                  <p className="text-xs text-text-secondary mb-1">{data?.name} · {data?.date}</p>
                  <p className="text-sm font-medium text-white">{data?.efficiency} W/bpm</p>
                  <p className="text-xs text-text-muted">{data?.watts}W @ {data?.hr} bpm</p>
                </div>
              );
            }}
          />
          <Area type="monotone" dataKey="efficiency" stroke="hsl(160, 60%, 45%)" strokeWidth={2.5} fill="url(#colorEfficiency)" dot={rides.length < 30 ? { r: 3, fill: "hsl(160, 60%, 45%)" } : false} />
        </AreaChart>
      </ResponsiveContainer>
    </ChartCard>
  );
